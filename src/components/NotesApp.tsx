...
export function NotesApp({ user, onDisconnect }: NotesAppProps) {
  ...
  const createNoteHandler = async (noteData: Omit<Note, 'id' | 'createdAt' | 'updatedAt'>) => {
    ...
    toast.success('📝 Block created!', {
      description: 'Preparing to submit to blockchain...',
      duration: 5000,
    });
  ...
  const deleteNoteHandler = async (noteId: string) => {
    ...
    toast.loading('🗑️ Deleting block...', {
      description: 'Submitting delete transaction...',
    });
    ...
    toast.success('🗑️ Block deleted!', {
      description: 'The block has been permanently deleted.',
      duration: 5000,
    });
  };
